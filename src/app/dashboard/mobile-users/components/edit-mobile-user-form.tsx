...
import { Institution, License, MobileUser, MobileUserStatus } from "@/lib/definitions";
import { updateMobileUser } from "@/lib/mobile-api";
import { useToast } from "@/components/ui/use-toast";

interface EditMobileUserFormProps {
...
export function EditMobileUserForm({ mobileUser, institutions, licenses }: EditMobileUserFormProps) {
  const router = useRouter();
  const { toast } = useToast();
  const [isSubmitting, setIsSubmitting] = useState(false);
  
  // 기관별 라이선스 필터링
  const [selectedInstitutionId, setSelectedInstitutionId] = useState<string>(mobileUser.institutionId);
  const filteredLicenses = selectedInstitutionId 
    ? licenses.filter(license => license.institutionId === selectedInstitutionId && (license.status === "active" || license.id === mobileUser.licenseId))
    : [];
  ...
  const form = useForm<FormValues>({
    ...
    defaultValues: {
      ...
      institutionId: mobileUser.institutionId,
      licenseId: mobileUser.licenseId,
      deviceId: mobileUser.deviceId,
      usageCount: mobileUser.usageCount,
      totalCount: mobileUser.totalCount,
  ...
  const onSubmit = async (data: FormValues) => {
    try {
      ...
      // 사용자 수정 API 호출
      await updateMobileUser(mobileUser.id, {
        name: data.name,
      ...
      toast({
        title: "Mobile앱 사용자 수정 완료",
        description: `${data.name} 사용자 정보가 수정되었습니다.`,
      });
      ...
      console.error("Mobile앱 사용자 수정 오류:", error);
      toast({
        title: "Mobile앱 사용자 수정 실패",
        description: "사용자 수정 중 오류가 발생했습니다. 다시 시도해주세요.",
        variant: "destructive",
      });
    } finally {
